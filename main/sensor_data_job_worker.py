from settings import SENSOR_OUT_PATH, USE_SENSOR, FAKE_GPIO, SENSOR_NUM


if not USE_SENSOR or FAKE_GPIO:
    print('Its not raspberry or sensor disabled in settings')
else:
    import time
    import json
    import pigpio
    ...
    STATUS_V = {DHT_GOOD: 'a good reading',
                DHT_BAD_CHECKSUM: 'receieved data failed checksum check',
                DHT_BAD_DATA: 'data receieved had one or more invalid values',
                DHT_TIMEOUT: 'no response from sensor'}
    ...
    class Sensor:
        ...
        def _datum(self):
            return ((int(self._timestamp), self._gpio, self._status,
    ...
    if __name__ == '__main__':
        while True:
            try:
                pi = pigpio.pi()
                s = Sensor(pi, SENSOR_NUM)
                timestamp, pin_num, status, temper_c, humidity = s.read()
                temper_f = temper_c * (9 / 5) + 32
                data = {"temp":  temper_c,
                        "temp_f": round(temper_f, 1),
                        "humidity": humidity,
                        "last_time": timestamp,
